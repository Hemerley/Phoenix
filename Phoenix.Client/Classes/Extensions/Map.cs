...
namespace Phoenix.Client.Classes.Extensions
{
    public class Map : PictureBox
    {
		...
		private Dictionary<int, Brush> roomBrushes = new Dictionary<int, Brush>();

		/// <summary>The pen to draw the lines showing you can go that direction.</summary>
		private Pen linePen = new Pen(Brushes.White);
		...
		public void DrawMap(Room[,] rooms)
		{
			...
			Bitmap bm = new(this.Size.Width, this.Size.Height);
			using (var g = Graphics.FromImage(bm))
			{
				g.Clear(this.BackColor);

				var roomGap = (int)(this.Size.Width * 0.05);
				var roomsWide = rooms.GetLength(0);
				var roomsHigh = rooms.GetLength(1);
